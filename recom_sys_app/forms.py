...
class UserProfileForm(forms.ModelForm):
    ...
    favourite_genre1 = forms.ChoiceField(choices=Genre.choices, required=False, label="Favourite Genre 1")
    favourite_genre2 = forms.ChoiceField(choices=Genre.choices, required=False, label="Favourite Genre 2")

    class Meta:
        model = UserProfile
        fields = [
            "name", "sex", "age", "date_of_birth", "country",
            "favourite_genre1", "favourite_genre2",
            "liked_g1_tmdb_id", "liked_g1_title",
            "liked_g2_tmdb_id", "liked_g2_title",
            "language", "timezone", "onboarding_complete",
        ]
        widgets = {
...
class SignUpForm(UserCreationForm):
    ...
    country = forms.CharField(max_length=64, label="Country")  # ← NEW (required by model)

    age = forms.IntegerField(min_value=1, max_value=120, required=False)
    sex = forms.ChoiceField(choices=Sex.choices, required=True, label="Sex")
    favourite_genre1 = forms.ChoiceField(choices=Genre.choices, required=False, label="Favourite Genre 1")
    favourite_genre2 = forms.ChoiceField(choices=Genre.choices, required=False, label="Favourite Genre 2")
    liked_g1_title = forms.CharField(max_length=300, required=False, label="Movie you like in Genre 1")
    liked_g2_title = forms.CharField(max_length=300, required=False, label="Movie you like in Genre 2")
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            UserProfile.objects.get_or_create(
                ...
                defaults={
                    "name": name,
                    "country": self.cleaned_data.get("country", "").strip(),  # ← PASS COUNTRY
                    ...
                    "liked_g1_title": (self.cleaned_data.get("liked_g1_title") or "").strip(),
                    "liked_g2_title": (self.cleaned_data.get("liked_g2_title") or "").strip(),
                },
            )
        return user
