...
def solo_genre_selection(request):
    ...
    return render(request, 'recom_sys_app/solo_genre_selection.html', {
        'genres': genres
    })
...
def solo_deck_view(request):
    ...
    return render(request, 'recom_sys_app/solo_deck.html', {
...
def solo_swipe(request):
    ...
    try:
        ...
        if not tmdb_id or not action:
            return JsonResponse({
                'success': False,
                'error': 'tmdb_id and action are required'
            }, status=400)
        ...
        try:
            ...
            for movie in results:
                ...
                movies.append({
                    ...
                    "backdrop_url": (IMG_BASE + movie["backdrop_path"]) if movie.get("backdrop_path") else None,
